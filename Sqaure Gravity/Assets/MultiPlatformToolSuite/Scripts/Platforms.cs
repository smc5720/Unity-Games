...
public enum Platform {iPhone=1, WebPlayer=2, iPad=3, iPhoneRetina=4, Standalone=5, Android=6, FlashPlayer=7, NaCl=8, iPadRetina=9, iPhone5=10, WinPhone=11, Windows8=12};
...
public class Platforms : MonoBehaviour {
	...
	public static Platform platform {
		get {
			...
			else if(platformString == Platform.WinPhone.ToString()) {
				calculatedPlatform = Platform.WinPhone;
			}
			...
			if(!platformCalculated) { //If platform wasn't calculated before, calculate now
				if(Application.platform == RuntimePlatform.IPhonePlayer) {
					...
					calculatedPlatform = Platform.WinPhone; //exact screen size will be calculated per-Aspect Ratio
				}
	...
	public static bool IsPlatformAspectBased(string plat) {
		return plat == Platform.Standalone.ToString() 
			...
			|| plat == Platform.WinPhone.ToString() 
	...
	public static bool IsiOS {
		get {
			return (Platforms.platform == Platform.iPad || Platforms.platform == Platform.iPhone || Platforms.platform == Platform.iPadRetina || Platforms.platform == Platform.iPhoneRetina || Platforms.platform == Platform.iPhone5); 
		}
	}
