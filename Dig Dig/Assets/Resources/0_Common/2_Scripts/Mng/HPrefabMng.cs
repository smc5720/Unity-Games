...
public class HPrefabMng : MonoBehaviour
{
    ...
    public GameObject CreatePrefab2(string sParentName, E_H_RESOURCELOAD eResourceLoadPos, string sPrefabName, Vector3 PosVec, Vector3 ScaleVec, string sTitleName = "", string sAddComponent = "")
    {
        GameObject ParentGame = GameObject.Find(sParentName);
        GameObject CretaePrefabGame = null;
            ...
            if (Resource != null)
            {
                foreach (GameObject obj in Resource)
                {
                    if (obj.transform.name == sPrefabName)
                    {
                        ...
                        if (sTitleName != "")
                            CretaePrefabGame.transform.name = sTitleName;
